...
_.first = function(arr, num){
    if(_.typeOf(arr) !== "array" || num < 0) return [];
    if(_.typeOf(num) !== "number") return arr[0];
...
_.last = function(arr, num){
    ...
    if(_.typeOf(num) !== "number" || num === undefined) return arr[arr.length - 1];
...
_.filter = function(arr, fx){
    var results = [];
    _.each(arr, function(value, index, col){
...
*/
_.partition = function(arr, fx){
    var truthy = _.filter(arr, fx);
    var falsy = _.reject(arr, fx);
    return [truthy, falsy];
}
...
_.unique = function (arr){
    ...
    _.each(arr,function(val){
        if(_.indexOf(results, val) === -1) results.push(val);
    });
    return results;
}
...
_.pluck = function (arr, property){
    var plucked = _.map(arr,function(value, i, array){
...
_.every = function (collection, fx) {
    var booleans = [];
    ...
    return _.contains(booleans, false) ? false : true;
};
...
_.reduce = function (arr, fx, seed) {
    // if(seed === undefined){
    //     var reduced = arr[0];
    //     arr.splice(0, 1, 1);
    //     //this should not be the case. Is it a problem with the test?
    // } else {
    //     var reduced = seed;
    // }
